...
class FirebaseDatabase {
  ...
  //Constructor for getting the UID.
  ...
  Future<void> initialSaveUserToCollection() async {
    ...
    try {
      ...
      final CollectionReference seniorCollectionReference = _firestore.collection('seniors');
      DocumentSnapshot seniorDocument = await seniorCollectionReference.document(this.uid).get();
      Map<String, dynamic> seniorMap = seniorDocument.data;

      // Checking if data is already present. If not, create a document.
      if (seniorMap == null) {
        await seniorCollectionReference.document(this.uid).setData({
          "uid": this.uid,
